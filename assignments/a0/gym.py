...
class Instructor:
    """An instructor that may teach classes in the gym

    === Public Attributes ===
    name: The name of the instructor.

    === Private Attributes ===
    _id: ID number of the instructor.
    _cert: list of certificates instructor has.
    """
    name: str
    _id: int
    _cert: list[str]

    def __init__(self, idnum: int, name: str) -> None:
        """Initialize a new Instructor called <name> and with the id
        <idnum>.

        >>> instructor = Instructor(81, 'Bob')
        >>> instructor.name
        'Bob'
        """
        self.name = name
    ...
    def get_id(self) -> int:
        """ Return the id that the instructor received.
    ...
    def get_certificates(self) -> list[str]:
        """ Return the certificates that the instructor has.
        >>> instructor = Instructor(1, 'Diane')
    ...
    def add_certificate(self, cert: str) -> bool:
        """ Add a certificate to the list of certificates Instructor has.
        >>> instructor = Instructor(1, 'Diane')
        >>> instructor.add_certificate('Training')
        True
        """
        if cert not in self._cert:
            self._cert += [cert]
            return True
        else:
            return False
...
class Gym:
    ...
    _schedule: dict[
        datetime, dict[str, tuple[Instructor, WorkoutClass, list[str]]]]
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        if time_point in self._schedule:
            if room_name not in self._schedule[time_point]:
                x += 1
            for item in self._workouts[workout_name]. \
                    get_required_certificates():
                if item not in self._instructors[instr_id].get_certificates():
                    x += 20
            for room in self._schedule[time_point]:
                if self._schedule[time_point][room][0] == \
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        if time_point in self._schedule:
            for room in self._schedule[time_point]:
                space = self._room_capacities[room] - len(
                    self._schedule[time_point][room][2])
                if self._schedule[time_point][room][1] \
                        == self._workouts[workout_name] and space > 0:
                    possible_rooms[space] = room
                if client in self._schedule[time_point][room][2]:
                    return False
            self._schedule[time_point][possible_rooms[max(possible_rooms)]][
                2].append(client)
            return True
        else:
            return False
        ...
        for time in self._schedule:
            if time1 <= time <= time2:
                for room in self._schedule[time]:
                    x += 1
                    instr_hrs[self._schedule[time][room][0].get_id()] = x

        for instrid in self._instructors:
            if instrid not in instr_hrs:
                instr_hrs[instrid] = 0
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        payroll = []
        instr_hrs = self.instructor_hours(time1, time2)
        for instrid in instr_hrs:
            bonus = BONUS_RATE * len(self._instructors[instrid].
                                     get_certificates())
            payroll.append((instrid, self._instructors[instrid].name,
                            instr_hrs[instrid],
                            instr_hrs[instrid] * (base_rate + bonus)))
        payroll.sort()
        return payroll
    ...
    def _is_instructor_name_unique(self, instructor: Instructor) -> bool:
        ...
        x = 0
        for instrid in self._instructors:
            if self._instructors[instrid].name == instructor.name:
                x += 1
        if x <= 1:
            return True
        else:
    ...
    def offerings_at(self, time_point: datetime) -> list[dict[str, str | int]]:
        ...
        if time_point in self._schedule:
            for room in self._schedule[time_point]:
                available = self._room_capacities[room] - len(
                    self._schedule[time_point][room][2])
                instr_name = self._schedule[time_point][room][0].name
                instr_id = self._schedule[time_point][room][0].get_id()

                if self._is_instructor_name_unique(
                        self._schedule[time_point][room][0]) is False:
                    instr_name = instr_name + ' (' + str(instr_id) + ')'

                offerings = create_offering_dict(
                    time_point.strftime("%A, %Y-%m-%d"),
                    time_point.strftime('%H:%M'),
                    self._schedule[time_point][room][1].name,
                    room, len(self._schedule[time_point][room][2]),
                    available,
                    instr_name)
                list_offerings.append(offerings)

        return sorted(list_offerings, key=lambda x: x['Room'])
    ...
    def to_schedule_list(self, week: datetime = None) \
        ...
        if week is None:
            for time_point in self._schedule:
                list_schedule.extend(self.offerings_at(time_point))
        else:
            for time_point in self._schedule:
                if in_week(time_point, week):
                    list_schedule.extend(self.offerings_at(time_point))

        list_schedule = sorted(list_schedule, key=lambda x: (
            datetime.strptime(x['Date'], '%A, %Y-%m-%d'),
            datetime.strptime(x['Time'], '%H:%M'),
            x['Room']))
        return list_schedule

    def _get_all_info(self) -> list[dict]:
        """Make all the private information accessible and return it in a list
        """
        info = [self._instructors.copy(), self._workouts.copy(),
                self._room_capacities.copy(), self._schedule.copy()]
        return info[:]
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        True
        """
        return self.name == other.name and self._get_all_info() == \
            other._get_all_info()
