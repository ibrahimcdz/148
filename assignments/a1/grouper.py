    ...
    def add_group(self, group: Group) -> bool:
        ...
        for grp in self._groups:
            for student in grp.get_members():
                if student in group.get_members():
                    return False
...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        students = sort_students(course.students[:], 'name')
        alpha_grouping = Grouping()

        temp = slice_list(students, self.group_size)
        for group in temp:
            alpha_grouping.add_group(Group(group))
        return alpha_grouping
...
class GreedyGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        Preconditions:
            - <course> has more students than this Grouper's group_size
        """
        greed_grouping = Grouping()
        students = list(course.get_students())

        while students:
            group = [students[0]]
            students.pop(0)
            while len(group) < self.group_size and students != []:
                best = find_best_addition_to_group(survey, group, students)
                group.append(best)
                students.remove(best)
            greed_grouping.add_group(Group(group))
        return greed_grouping
...
class SimulatedAnnealingGrouper(Grouper):
    ...
    === Private Attributes ===
    _iterations: the number of iterations that this grouper runs for
    _temperature: the measure of how close the grouper is to best possible
    group
    ...
    def __init__(self, group_size: int, iterations: int = 10 ** 4,
        ...
        if initial_temperature < 1:
            raise ValueError('Temperature must be greater than or equal to 1')
        if iterations < 1:
            raise ValueError('You must iterate at least once')
        if group_size <= 1:
            raise ValueError('Group is too small')
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        Preconditions:
            - <course> has more students than this Grouper's group_size
        """
        sim_grouping = Grouping()
        best = []
        sorted_s = sort_students(list(course.get_students()), 'id')
        init_groups = slice_list(sorted_s, self.group_size)
        init_score = total_score(survey, init_groups)

        curr_score = init_score
        curr_groups = deepcopy(init_groups)

        # i = 0
        best_score = float('-inf')
        for i in range(self._iterations):
            random_swap(curr_groups, i)
            new_score = total_score(survey, curr_groups)

            if self._iterations == 1:
                temp_iterated = self._temperature
            else:
                temp_iterated = \
                    self._temperature * (1 - (i / (self._iterations - 1)))

            if curr_score > best_score:
                best_score = curr_score
                new_score = curr_score
                best = deepcopy(curr_groups)
                curr_groups = deepcopy(curr_groups)
            if accept(curr_score, new_score, temp_iterated, i):
                best = curr_groups
                curr_score = new_score
                curr_groups = deepcopy(curr_groups)

        for group in best:
            sim_grouping.add_group(Group(group))
        return sim_grouping
