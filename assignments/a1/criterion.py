    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        for i in range(len(answers)):
            for j in range(len(answers)):
                if i+j+1 < len(answers):
                    similarities += question.get_similarity(answers[i], answers[i+(j+1)])

        final_similarity = similarities / ((len(answers)*(len(answers)-1)) / 2)
...
class HeterogeneousCriterion(HomogeneousCriterion):
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        difference_score = 1.0 - HomogeneousCriterion().score_answers(question, answers)
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        for answer in answers:
            for answer2 in answers:
                if not answer2.content == answer.content:
                    return 0.0
        return 1.0
