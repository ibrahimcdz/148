...
def mergesort3(lst: list) -> list:
    ...
    if len(lst) < 2:  # We've provided the base case for you.
        ...
        third = len(lst) // 3
        secthird = (len(lst) // 3) * 2

        first_sorted = mergesort3(lst[:third])
        second_sorted = mergesort3(lst[third:secthird])
        third_sorted = mergesort3(lst[secthird:])
...
def merge3(lst1: list, lst2: list, lst3: list) -> list:
    ...
    if index1 == len(lst1):  # lst2 and lst3 have remaining terms
