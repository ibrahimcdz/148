...
class BinarySearchTree:
    ...
    def maximum(self) -> Optional[int]:
        ...
        if self.is_empty():
            ...
            # there is no left so biggest value is in root
            return self._root
        else:
            # know that there is a left branch with bigger values
    ...
    def items(self) -> list:
        ...
        if self.is_empty():
            ...
            if not self._left.is_empty():
                s.extend(self._left.items())
            s.append(self._root)
            if not self._right.is_empty():
                s.extend(self._right.items())
        return s
    ...
    def smaller(self, item: Any) -> list:
        ...
        if self.is_empty():
            return []
        if self._left.is_empty() and self._right.is_empty() and \
                self._root < item:
            return [self._root]
        else:
            if item < self._root:
                if not self._left.is_empty():
                    s.extend(self._left.smaller(item))
            else:
                if not self._left.is_empty() and self._root < item:
                    s.extend(self._left.smaller(item))
                    s.append(self._root)
                elif self._left.is_empty() and self._root < item:
                    s.append(self._root)
                elif self._root == item:
                    s.extend(self._left.smaller(item))
                if not self._right.is_empty() and self._root < item:
                    s.extend(self._right.smaller(item))
            return s
