...
class Tree:
    ...
    def swap_down(self) -> None:
        ...
        5
          ...
          4
        """
        root_init = self._root  # keep a copy of root value
        if self._subtrees:
            child_values = []
            for subtree in self._subtrees:
                child_values.append(subtree._root)

            for subtree in self._subtrees:
                if self._root < max(child_values) == subtree._root:
                    self._root = max(child_values)
                    subtree._root = root_init
                    subtree.swap_down()
